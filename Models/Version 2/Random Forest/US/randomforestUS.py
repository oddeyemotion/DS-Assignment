...
# load the dataset
globalactive = pd.read_csv('USActive.csv')
globalactive = globalactive.drop(labels='Date', axis=1)

series = pd.read_csv('USActive copy.csv', header=0, index_col=0)
...
series2 = pd.read_csv('USActive.csv', header=0, index_col=0)
...
plt.figure(figsize=(10, 5))
plt.plot(predic2[-15:])
plt.title('Prediction for Active cases in the next 14 days from 13-12-2021')
plt.show()


###########
# NOTE: Because of vague definitions of "recovered" and the fact that some states do not give data correctly, the provider of our dataset decided to stop getting the data on the recovered cases of US from 13-12-2020
# Therefore, we can only use data up until 13-12-2020
